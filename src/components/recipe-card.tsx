  ...
  return (
    ...
    >
      <a
        ...
        className={`group block ${viewMode === "list" ? "flex flex-1 items-stretch" : ""}`}
      >
        {/* Thumbnail */}
        <div
          className={
      ...
      {onCooked && (
        <div className="px-4 pb-4">
          <button
            onClick={() => onCooked(recipe.id)}
            className="w-full rounded-lg bg-accent-sage/20 px-3 py-1.5 text-sm font-medium text-accent-sage-light transition-colors hover:bg-accent-sage/30"
          >
            Cooked
          </button>
        </div>
